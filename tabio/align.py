...
def align(reference, hypothesis):
    D, B = _wagner_fischer(reference, hypothesis)
    bt = _naive_backtrace(B)
    return _align(reference, hypothesis, bt)
...
if __name__ == '__main__':
    ...
    alignment = align(word_1, word_2)
    print("Minimum edit distance with backtrace:")

    print("\nAlignment:")
    print(tb.tabulate(alignment, tablefmt="orgtbl"))


    print('------------------')

    a = ['alpha', 'bravo', 'charlie', 'delta', 'foxtrot']
    b = ['bravo', 'charles', 'delta', 'echo', 'foxtrot', 'golf']
